function [estSig, cost] = ILRMA(mixSig, nSrc, sampFreq, nBases, fftSize, shiftSize, windowType, nIter, ilrmaType, refMic, applyNormalize, applyWhitening, drawConv)
...
% Apply ILRMA
if ilrmaType == 1
    [estSpecgram, cost] = local_ILRMA1(inputMixSpecgram, nIter, nBases, applyNormalize, drawConv, mixSpecgram(:,:,refMic));
else
    [estSpecgram, cost] = local_ILRMA2(inputMixSpecgram, nIter, nBases, applyNormalize, drawConv);
end
...
%% Local function for ILRMA type1 (without pertitioning function)
function [Y, cost] = local_ILRMA1(X, nIter, L, applyNormalize, drawConv, refMixSpecgram)
% [inputs]
...
%% Local function for ILRMA type2 (with pertitioning function)
function [Y, cost] = local_ILRMA2(X, nIter, K, applyNormalize, drawConv)
% [inputs]
%              X: observed multichannel spectrograms (I x J x M)
%          nIter: number of iterations of the parameter updates
